...
SUMSUB_SECRET_KEY = (
    "YOUR_SUMSUB_SECRET_KEY"  # Example: Hej2ch71kG2kTd1iIUDZFNsO5C1lh5Gq
)
...
def create_applicant(external_user_id, level_name):
    ...
    resp = sign_request(
        requests.Request(
            "POST",
            SUMSUB_TEST_BASE_URL + "/resources/applicants?levelName=" + level_name,
            params=params,
...
def get_access_token(external_user_id, level_name):
    ...
    resp = sign_request(
        requests.Request(
            "POST",
            SUMSUB_TEST_BASE_URL + "/resources/accessTokens",
            params=params,
            headers=headers,
        )
    )
